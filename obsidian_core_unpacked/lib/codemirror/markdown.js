...
(function(mod) {
	...
	CodeMirror.defineMode("markdown", function(cmCfg, modeCfg) {
		...
		function enterFencedCodeblock(stream, state, match, inList) {
			// The terminating code fence must indented within the range of these min and max
			// values to be considered a valid termination
			state.fencedEndMinIndent = state.baseIndentation
			state.fencedEndMaxIndent = inList ?
				state.listStack.length * stream.indentUnit :
				state.baseIndentation;

			state.quote = 0;
			state.fencedEndRE = new RegExp('^(\\s*)' + match[1] + "+ *$");
			// try switching mode
		...
		function blockNormal(stream, state) {
			...
			if (state.indentationDiff >= 4 && (prevLineIsIndentedCode || state.prevLine.fencedCodeEnd ||
				...
				enterFencedCodeblock(stream, state, match);
				return getType(state);
		...
		function local(stream, state) {
			if (state.fencedEndRE) {
				...
				var hasExitedList = state.indentation <= maxFencedEndInd && state.indentation < currListInd;
				var maxFencedEndInd = currListInd + 3;

				// Check for the terminating code fence at the start of each line
				var match = stream.sol() && stream.match(state.fencedEndRE, false);
				var indentation = match ? match[1].replace(/\t/g, expandedTab).length : 0;
				// Validate that the fence is within the required indentation range
				var inRange = match && indentation >= state.fencedEndMinIndent && indentation <= state.fencedEndMaxIndent;

				if (hasExitedList || inRange) {
					// We've matched a terminating code fence which takes up a whole line, so move
		...
		function getType(state) {
			...
			// Don't apply code styles to blockquote formatting. We encounter this edge case with
			// codeblocks in blockquotes
			let isQuoteFormatting = state.formatting && state.formatting.includes('quote');
			if (state.code && !isQuoteFormatting) { styles.push(tokenTypes.code); }
		...
		function inlineNormal(stream, state) {
			var firstTokenInQuote = state.quoteOpen;
			...
			var inList = state.list !== false;
			// inTaskList is only ever true *right after* the checkbox markup has been processed above
			var inTaskList = state.taskOpen || state.taskClosed;
			var inFirstListColumn = stream.column() === state.indentation;
			var firstTokenInList = inList && (inFirstListColumn || inTaskList);
			var maxNonCodeIndentation = (state.listStack[state.listStack.length - 1] || 0) + 3;

			state.taskOpen = false;
			state.taskClosed = false;

			// Check for fenced codeblocks at the beginning of a list item
			if (firstTokenInList && state.indentation <= maxNonCodeIndentation && (match = stream.match(fencedCodeRE, true))) {
				enterFencedCodeblock(stream, state, match, true);
				return getType(state);
			}

			// Check for fenced codeblocks at the beginning of a blockquote line
			if (firstTokenInQuote && (match = stream.match(fencedCodeRE, true))) {
				// Found closing code fence
				if (state.code === 3) {
					var t = getType(state);
					state.code = 0;
					return t;
		...
		var mode = {
			startState: function() {
				return {
					...
					emoji: false,
					fencedEndRE: null,
					fencedEndMinIndent: 0,
					fencedEndMaxIndent: 0
				};
			},
			...
			copyState: function(s) {
				return {
					...
					baseIndentation: s.baseIndentation,
					...
					fencedEndMinIndent: s.fencedEndMinIndent,
					fencedEndMaxIndent: s.fencedEndMaxIndent
				};
			},
				...
				if (stream != state.thisLine.stream) {
					...
					if (!state.localState) {
						...
						if (state.f != htmlBlock) {
							...
							state.baseIndentation = indentation;
