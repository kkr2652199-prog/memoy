 ...
 * - 모든 DOM: #tab-lotto-reverse, #lotto2-* 접두사
 * - 7뇌 한글명: 1군 동일 6뇌 + V11 뱀 합성두뇌 (v11_snake)
 * - brain_tag: v11_* (DB 분리)
 */
...
// ── 탭 전환 ──
function switchLotto2Tab(tabName) {
  ...
  const target = document.getElementById('lotto2-tab-' + tabName);
  if (target) target.style.display = 'block';
  const btn = document.querySelector(`[data-lotto2-tab="${tabName}"]`);
  if (btn) btn.classList.add('active');

  if (tabName === 'predictions') {
    initLottoDrawSearch();
  }
  if (tabName === 'stats') loadLotto2Stats();
  if (tabName === 'draws') loadLotto2Draws();
}
...
let _lotto2DetailDrawNo = null;
let _lotto2DetailRows = null;
let _lotto2PredFetchSeq = 0;
let _currentBrainTab = 'v11_hyena';

/** 역대 고적중 필터용 brain_tag 집합(null=미로드). */
let _brainEliteTagSet = null;

const BRAIN_LIST = [
  { tag: 'v11_stat', name: '통계', icon: '📊', color: '#3b82f6' },
  { tag: 'v11_markov', name: '마르코프', icon: '🔗', color: '#10b981' },
  { tag: 'v11_combo', name: 'LLM', icon: '🤖', color: '#f59e0b' },
  { tag: 'v11_lstm', name: 'LSTM', icon: '🧠', color: '#ef4444' },
  { tag: 'v11_fusion', name: '퓨전', icon: '⚡', color: '#8b5cf6' },
  { tag: 'v11_hyena', name: '하이에나', icon: '🦁', color: '#eab308' },
  { tag: 'v11_snake', name: '뱀', icon: '🐍', color: '#059669' },
];

/** 대시보드 brain_power → 태그별 맵(2군 전용, 1군 캐시와 분리). */
let _lotto2BrainPowerByTag = null;

function setLotto2BrainPowerCache(brainPower) {
  const map = {};
  (brainPower || []).forEach((b) => {
    const k = String(b.brain || '').toLowerCase();
    if (k) map[k] = b;
  });
  _lotto2BrainPowerByTag = map;
}

async function ensureLotto2BrainPowerLoaded() {
  if (_lotto2BrainPowerByTag) return;
  try {
    const res = await fetch(_lottoResolveApiUrl('/api/lotto2/v11/dashboard-summary'));
    if (!res.ok) return;
    const data = await res.json();
    setLotto2BrainPowerCache(data.brain_power);
  } catch (e) {
    console.warn('lotto2 brain_power cache:', e);
  }
}

/** 역대 1~5등 적중 횟수 나노 한 줄(HTML). */
function lotto2BrainTierNanoHtml(tag) {
  const m = _lotto2BrainPowerByTag || {};
...
function lotto2KakaoBrainRecordLine(tag) {
  const m = _lotto2BrainPowerByTag || {};
  const b = m[String(tag).toLowerCase()] || {};
  const n1 = Number(b.rank1 || 0);
  const n2 = Number(b.rank2 || 0);
...
/** 세트 순번(#1~#5) → 카톡용 이모지 */
const _LOTTO2_KAKAO_SET_EMOJI = ['', '🥇', '🥈', '🥉', '4️⃣', '5️⃣'];
...
/** V11 POST 오류 reason → 사용자용 문구 */
function lotto2V11ErrorReasonText(reason) {
  const r = String(reason || '');
  if (r === 'insufficient_training_data') {
    return '이 회차 예측을 만들 이전 당첨 데이터가 부족합니다. 최신 회차 이하로 선택하거나 당첨 데이터를 먼저 수집하세요.';
  }
  if (r === 'no_sets_generated') {
    return '예측 조합을 생성하지 못했습니다. 다른 회차로 시도하세요.';
  }
  return r || '예측 오류';
}
...
async function loadLottoDrawList() {
  try {
    const r = await fetch(resolveApiUrl('/api/lotto2/v11/predictions?limit=20000'));
    ...
    const dr = await fetch(resolveApiUrl('/api/lotto2/v11/draws?limit=10000'));
    ...
    draws.forEach((d) => {
      if (d && d.draw_no != null && d.draw_no !== '') {
        ...
        if (Number.isFinite(n) && n > 0) {
          set.add(n);
          _lottoDrawDates[n] = d.draw_date;
        }
      }
    });
    _lottoDrawList = Array.from(set).filter((n) => Number.isFinite(n) && n > 0).sort((a, b) => b - a);

    refreshLotto2DrawSelectDom();
    return true;
  } catch (e) {
...
function refreshLotto2DrawSelectDom() {
  const sel = document.getElementById('lotto2DrawSelect');
 ...
 */
function _lotto2ActiveDrawIndex(baseDraw) {
...
function lotto2SelectDraw(drawNo) {
  const no = parseInt(drawNo, 10);
  if (!no) return;
  const input = document.getElementById('lotto2PredictDrawNo');
  if (input) input.value = String(no);
  lottoLoadSavedPrediction(no, { softLoading: true });
}

function lotto2NavDraw(delta) {
  ...
  const navIdx = _lotto2ActiveDrawIndex(base);
  ...
  lottoLoadSavedPrediction(nextNo, { softLoading: true });
}

async function lottoLoadSavedPrediction(drawNo, options) {
  const d = parseInt(drawNo, 10);
  const fromTab = options && options.fromTab;
  const softLoading = options && options.softLoading;
  const container = document.getElementById('lotto2PredictionResults');
  if (!container || !Number.isFinite(d) || d < 1) {
    return;
  }
  if (fromTab && _lotto2DetailDrawNo === d && Array.isArray(_lotto2DetailRows) && _lotto2DetailRows.length) {
    await renderPredictionsByBrain(d, _lotto2DetailRows);
    return;
  }
  const hasRichUi = !!container.querySelector('.lotto-brain-tabs, .lotto-result-header');
  const useSoftShell = !!(softLoading && hasRichUi);
  const seq = ++_lotto2PredFetchSeq;
  if (useSoftShell) {
    container.classList.add('lotto2-results-pending');
    container.setAttribute('aria-busy', 'true');
  } else {
    container.classList.remove('lotto2-results-pending');
    container.removeAttribute('aria-busy');
    container.innerHTML = '<p style="color: #888;">로딩 중...</p>';
  }
  try {
    const r = await fetch(resolveApiUrl('/api/lotto2/v11/predictions/draw/' + d));
    const data = await r.json();
    if (seq !== _lotto2PredFetchSeq) {
      return;
    }
    const rows = (data && data.predictions) ? data.predictions : [];
    if (!rows.length) {
      _lotto2DetailDrawNo = null;
      _lotto2DetailRows = null;
      container.classList.remove('lotto2-results-pending');
      container.removeAttribute('aria-busy');
      container.innerHTML = `<p style="color: #888;">${d}회차 저장된 예측 없음. \"두뇌 예측\" 버튼으로 실행하세요.</p>`;
      return;
    }
    _lotto2DetailDrawNo = d;
    _lotto2DetailRows = rows;
    _lottoPredRowsCache = (_lottoPredRowsCache || []).filter((p) => parseInt(p.target_draw_no, 10) !== d);
...
function lotto2SwitchBrainTab(brainTag) {
  _currentBrainTab = String(brainTag || '').toLowerCase();
  const drawNo = parseInt(document.getElementById('lotto2PredictDrawNo').value, 10);
  if (drawNo) {
    lottoLoadSavedPrediction(drawNo, { fromTab: true });
  }
}
...
async function lotto2EnsureBrainEliteTagsLoaded() {
  if (_brainEliteTagSet !== null) {
    return true;
  }
  try {
    const r = await fetch(_lottoResolveApiUrl('/api/lotto2/v11/brain/elite-tags'));
    if (!r.ok) {
      throw new Error(String(r.status));
    }
    const data = await r.json();
    _brainEliteTagSet = new Set((data.tags || []).map((t) => String(t).toLowerCase()));
    return true;
  } catch (e) {
    console.warn('lotto2 brain elite-tags:', e);
    return false;
  }
}

async function lotto2OnEliteBrainToggle() {
  _brainEliteTagSet = null;
  const d = _lotto2DetailDrawNo;
  const rows = _lotto2DetailRows;
...
async function renderPredictionsByBrain(drawNo, rows) {
  const container = document.getElementById('lotto2PredictionResults');
  if (!container) return;
  await ensureLotto2BrainPowerLoaded();
  container.classList.remove('lotto2-results-pending');
  container.removeAttribute('aria-busy');
  ...
  const eliteCb = document.getElementById('lotto2EliteBrainToggle');
  const eliteOn = !!(eliteCb && eliteCb.checked);

  let brainListForTabs = BRAIN_LIST;
  if (eliteOn) {
    const ok = await lotto2EnsureBrainEliteTagsLoaded();
    if (ok) {
      brainListForTabs = BRAIN_LIST.filter((b) => _brainEliteTagSet.has(b.tag));
    }
  }
  ...
  if (!_currentBrainTab || _currentBrainTab === 'legacy') {
    _currentBrainTab = 'v11_hyena';
  }
  const allowedTags = new Set(brainListForTabs.map((b) => b.tag));
  if (eliteOn && allowedTags.size && !allowedTags.has(_currentBrainTab)) {
    _currentBrainTab = brainListForTabs[0].tag;
  }
  if (!byBrain[_currentBrainTab]) {
    const firstTag = Object.keys(byBrain)[0];
    _currentBrainTab = firstTag || (brainListForTabs[0] && brainListForTabs[0].tag) || 'v11_hyena';
  }
  ...
  const tabsHtml = brainListForTabs.map((b) => {
    ...
    const nano = lotto2BrainTierNanoHtml(b.tag);
    return `
      <button class="lotto-brain-tab ${active} ${disabled}"
              data-brain="${b.tag}"
              style="--brain-color: ${b.color};"
              onclick="lotto2SwitchBrainTab('${b.tag}')"
              ${disabled ? 'disabled' : ''}>
  ...
  container.innerHTML = `
    ...
    <div class="lotto-brain-cards" id="lotto2BrainCards">${cardsHtml}</div>
  `;
}

/**
 * 2군: 현재 불러온 예측(_lotto2DetailRows)을 카카오톡용 텍스트로 복사.
 * 고적중 필터·뇌 탭 목록과 동일한 뇌 순서(BRAIN_LIST)로 전 뇌 출력.
 */
async function lotto2CopyKakaoText() {
  const rows = _lotto2DetailRows;
  const drawNo = _lotto2DetailDrawNo;
  if (!rows || !rows.length || !drawNo) {
    alert('먼저 회차를 선택한 뒤 "역전 두뇌 예측"을 눌러 예측을 불러오세요.');
    return;
  }
  ...
  brainListForTabs.forEach((b) => {
    ...
    body += '\n';
    body += getBrainDisplayName(b.tag);
    body += '\n';
    list.forEach((row, i) => {
      const idx = i + 1;
      const emoji = _LOTTO2_KAKAO_SET_EMOJI[idx] || '📌';
    ...
    body += lotto2KakaoBrainRecordLine(b.tag);
  });
  ...
  const clip = typeof lottoClipboardWriteText === 'function' ? lottoClipboardWriteText : null;
  if (!clip) {
    alert('클립보드 기능을 쓰려면 1군 스크립트(lotto.js)가 로드된 상태여야 합니다. 페이지를 새로고침하세요.');
    return;
  }
  try {
    await clip(text);
    ...
    console.warn('lotto2CopyKakaoText', e);
    alert('복사에 실패했습니다. 브라우저 권한·HTTPS를 확인하세요.');
  }
}

/** 2군: 1~5등 적중 요약 모달 — 읽기 전용 API */
let _lotto2TierWinsEscHandler = null;

function lotto2CloseTierWinsModal() {
  const modal = document.getElementById('lotto2TierWinsModal');
  if (modal) {
    modal.hidden = true;
    modal.setAttribute('aria-hidden', 'true');
  }
  if (_lotto2TierWinsEscHandler) {
    document.removeEventListener('keydown', _lotto2TierWinsEscHandler);
    _lotto2TierWinsEscHandler = null;
  }
}

function lotto2TierWinsRankTitleClass(rank) {
...
function lotto2RenderTierWinsModalContent(data) {
  const body = document.getElementById('lotto2TierWinsModalBody');
  const title = document.getElementById('lotto2TierWinsModalTitle');
  ...
  for (let r = 1; r <= 5; r++) {
    ...
    html += `<section class="lotto-tier-wins-rank"><h3 class="${lotto2TierWinsRankTitleClass(r)}">${rankTitles[r]}</h3><ul class="lotto-tier-wins-list">`;
    list.forEach((it) => {
      const nums = (it.nums || []).map((n) => renderMiniBall(n, false)).join('');
      const tag = String(it.brain_tag || '').toLowerCase();
      const brain = getBrainDisplayName(tag);
...
async function lotto2RefreshTierWinsModalContent() {
  const input = document.getElementById('lotto2PredictDrawNo');
  const d = input ? parseInt(input.value, 10) : NaN;
  const body = document.getElementById('lotto2TierWinsModalBody');
  const title = document.getElementById('lotto2TierWinsModalTitle');
  const jump = document.getElementById('lotto2TierWinsJumpInput');
  if (!body) return;
...
async function lotto2OpenTierWinsModal() {
  ...
  await lotto2RefreshTierWinsModalContent();
  _lotto2TierWinsEscHandler = (e) => {
    if (e.key === 'Escape') lotto2CloseTierWinsModal();
  };
  document.addEventListener('keydown', _lotto2TierWinsEscHandler);
}

async function lotto2TierWinsModalNav(delta) {
  if (!_lottoDrawList.length) {
    await lotto2RefreshTierWinsModalContent();
    return;
  }
  ...
  if (input) input.value = String(nextNo);
  if (sel) sel.value = String(nextNo);
  await lottoLoadSavedPrediction(nextNo, { softLoading: true });
  await lotto2RefreshTierWinsModalContent();
}

async function lotto2TierWinsModalGoDraw() {
  const jump = document.getElementById('lotto2TierWinsJumpInput');
  ...
  await lottoLoadSavedPrediction(d, { softLoading: true });
  await lotto2RefreshTierWinsModalContent();
}
...
async function lotto2FetchAll() {
  const status = document.getElementById('lotto2CollectStatus');
  const btn = document.getElementById('btnLotto2FetchAll');
...
async function lotto2FetchLatest() {
  const status = document.getElementById('lotto2CollectStatus');
  if (!status) return;
  try {
...
async function lotto2Predict() {
  ...
  try {
    const res = await fetch(resolveApiUrl(`/api/lotto2/v11/predict/${drawNo}`), { method: 'POST' });
    const data = await res.json();
    ...
    if (data.status === 'error') {
      lottoSetActionStatusText(status, lotto2V11ErrorReasonText(data.reason), '#f87171');
      return;
    }
    ...
    try {
      ...
      if (sel) {
        ...
        if (rawPreds.length > 0) {
          ...
          try {
            const pr = await fetch(resolveApiUrl('/api/lotto2/v11/predictions/draw/' + drawNo));
            const pj = await pr.json();
            rows = (pj && pj.predictions) ? pj.predictions : [];
          } catch (_e) {
...
async function loadLotto2BrainStatus() {
  try {
    var res = await fetch(resolveApiUrl('/api/lotto2/v11/brain/status'));
    var data = await res.json();

    document.getElementById('lotto2BrainGrade').textContent = data.grade_emoji || '🧠';
    document.getElementById('lotto2BrainGradeText').textContent = '두뇌 등급: ' + (data.grade || '일반');
    ...
    document.getElementById('lotto2BrainStats').innerHTML = statsHtml;

    // 3종 두뇌별 성적 비교 카드
    var profileDiv = document.getElementById('lotto2BrainProfiles');
...
function ensureLotto2FameClickDelegation() {
  if (window._lotto2FameDelegationBound) {
    return;
  }
  var root = document.getElementById('lotto2HallOfFame');
  if (!root) {
    return;
  }
  window._lotto2FameDelegationBound = true;
  root.addEventListener('click', function (ev) {
    var filterBtn = ev.target && ev.target.closest && ev.target.closest('[data-lotto2-fame-rank]');
    if (filterBtn && root.contains(filterBtn)) {
      var rank = filterBtn.getAttribute('data-lotto2-fame-rank');
      if (rank != null) {
        fameFilterRank(rank);
      }
      ev.preventDefault();
    ...
    var moreBtn = ev.target && ev.target.closest && ev.target.closest('[data-lotto2-fame-brain]');
    if (moreBtn && root.contains(moreBtn)) {
      var brain = moreBtn.getAttribute('data-lotto2-fame-brain');
      if (brain) {
        fameShowMore(brain);
      }
      ev.preventDefault();
...
async function loadLotto2HallOfFame() {
  try {
    ensureLotto2FameClickDelegation();
    const res = await fetch(resolveApiUrl('/api/lotto2/v11/brain/hall-of-fame?limit=25000'));
    const data = await res.json();
...
function renderHallOfFame() {
  var container = document.getElementById('lotto2HallOfFame');
  ...
  ranks.forEach(function(rk) {
    ...
    html += '<button type="button" class="fame-filter-btn' + active + '" data-lotto2-fame-rank="' + rk.key + '">' + rk.label + '</button>';
  });
  html += '</div>';

  // 6컬럼 그리드
  var brains = ['v11_stat', 'v11_markov', 'v11_combo', 'v11_lstm', 'v11_fusion', 'v11_hyena', 'v11_snake'];
  ...
  brains.forEach(function(brain) {
    ...
    if (brainData.length > limit) {
      ...
      html += '<button type="button" class="fame-more-btn" data-lotto2-fame-brain="' + brain + '">+ 더보기 (' + remaining + '건 남음)</button>';
    }
...
// ── 통계 분석 ──
async function loadLotto2Stats() {
  try {
    const res = await fetch(resolveApiUrl('/api/lotto2/v11/stats/comprehensive'));
    const data = await res.json();

    if (data.error) {
      document.getElementById('lotto2FreqChart').innerHTML = `<p style="color: #f87171;">${data.error}</p>`;
      return;
    }
...
function renderFreqChart(freq) {
  const container = document.getElementById('lotto2FreqChart');
...
function renderOddEvenChart(data) {
  const container = document.getElementById('lotto2OddEvenChart');
...
function renderRangeChart(data) {
  const container = document.getElementById('lotto2RangeChart');
  if (!data) { container.innerHTML = '데이터 없음'; return; }
...
function renderSumChart(data) {
  const container = document.getElementById('lotto2SumChart');
  if (!data) { container.innerHTML = '데이터 없음'; return; }
...
function renderPairChart(pairs) {
  const container = document.getElementById('lotto2PairChart');
...
async function loadLotto2Draws() {
  try {
    const res = await fetch(resolveApiUrl('/api/lotto2/v11/draws?limit=50'));
    const data = await res.json();
    const container = document.getElementById('lotto2DrawsList');
...
document.addEventListener('DOMContentLoaded', () => {
  const reverseTab = document.querySelector('[data-tab="lotto-reverse"]');
  if (reverseTab) {
    reverseTab.addEventListener('click', () => {
      setTimeout(() => {
        loadLotto2Dashboard();
        loadLotto2BrainStatus();
        loadLotto2HallOfFame();
        initLottoDrawSearch();
      }, 300);
    });
  }
...
// === 사이드바 페이지 전환 ===
function switchLotto2Page(pageName) {
  const root = document.getElementById('tab-lotto-reverse');
  if (!root) return;
  ...
  const page = document.getElementById('lotto2-page-' + pageName);
  if (page) page.classList.add('active');

  root.querySelectorAll('.lotto-sidebar-item').forEach((b) => {
    if (b.getAttribute('onclick') === "switchLotto2Page('" + pageName + "')") {
  ...
  if (pageName === 'fame') loadLotto2HallOfFame();
  if (pageName === 'stats') loadLotto2Stats();
  if (pageName === 'draws') loadLotto2Draws();
  if (pageName === 'brains') loadLotto2BrainStatus();
}

/** 역전 대시보드 랭킹 패널 ID: index.html `lotto2Rank1List` / `lotto2Rank1Count` */
function _lotto2RankPanelId(rankKey, which /* 'List' | 'Count' */) {
  const mid = String(rankKey || '').replace(/^rank/i, 'Rank'); // rank1 -> Rank1
  return 'lotto2' + mid + which;
}

// === 등수 드롭다운 토글 ===
function toggleLotto2RankDropdown(rankId) {
  const list = document.getElementById(_lotto2RankPanelId(rankId, 'List'));
  if (list) {
...
async function loadLotto2Dashboard() {
  try {
    const res = await fetch('/api/lotto2/v11/dashboard-summary');
    const data = await res.json();
    setLotto2BrainPowerCache(data.brain_power);
...
function renderCountdown(drawNo, dateStr, weekday) {
  const nextEl = document.getElementById('lotto2NextDraw');
  if (nextEl) {
  ...
  function updateTimer() {
    const el = document.getElementById('lotto2CountdownTimer');
    if (!el) return;
    const now = new Date();
...
function renderRankings(rankings) {
  if (!rankings) return;
  window._lotto2DashboardRankings = rankings;
  if (!window._lotto2RankState) {
    window._lotto2RankState = { rank1: { limit: 20 }, rank2: { limit: 20 }, rank3: { limit: 20 } };
  }
  ['rank1', 'rank2', 'rank3'].forEach((key) => {
    const list = rankings[key] || [];
    const countEl = document.getElementById(_lotto2RankPanelId(key, 'Count'));
    const listEl = document.getElementById(_lotto2RankPanelId(key, 'List'));
    ...
    if (listEl) {
      if (list.length === 0) {
        ...
        const state = window._lotto2RankState[key] || { limit: 20 };
        ...
        if (list.length > limit) {
          ...
          html += '<button class="btn btn-primary" style="padding:8px 12px;font-size:12px" onclick="lotto2RankShowMore(\'' + key + '\')">더보기 (+20)</button>';
          html += '</div>';
        } else {
...
function lottoRankShowMore(key) {
  if (!window._lotto2RankState) window._lotto2RankState = {};
  const state = window._lotto2RankState[key] || { limit: 20 };
  state.limit = Number(state.limit || 20) + 20;
  window._lotto2RankState[key] = state;
  if (window._lotto2DashboardRankings) {
    renderRankings(window._lotto2DashboardRankings);
    const el = document.getElementById(_lotto2RankPanelId(key, 'List'));
...
function renderPowerMeter(brainPower) {
  const el = document.getElementById('lotto2PowerMeterContent');
...
function renderProgress(range, totalPreds) {
  const el = document.getElementById('lotto2ProgressContent');
  if (!el || !range) return;
...
function renderScores(scores) {
  const el = document.getElementById('lotto2ScoresContent');
  if (!el || !scores) return;
  const rows = [
...
(function() {
  const lottoTab = document.getElementById('tab-lotto-reverse');
  if (!lottoTab || typeof MutationObserver === 'undefined') return;
  const observer = new MutationObserver(function(mutations) {
    mutations.forEach(function(m) {
      if (m.target && m.target.id === 'tab-lotto-reverse' && m.target.classList.contains('active')) {
        loadLotto2Dashboard();
      }
    });
  });
...
function lottoGetActiveStatusEl() {
  const reverse = document.getElementById('tab-lotto-reverse');
  if (!reverse) return document.getElementById('lotto2ActionStatus');
  const activePage = reverse.querySelector('.lotto-page.active');
  if (!activePage) return document.getElementById('lotto2ActionStatus');
  if (activePage.id === 'lotto2-page-collect') {
    return document.getElementById('lotto2CollectStatus') || document.getElementById('lotto2ActionStatus');
  }
  return document.getElementById('lotto2ActionStatus');
}

  // 전역 노출 (index.html onclick 호환)
  window.switchLotto2Page = switchLotto2Page;
  window.switchLotto2Tab = switchLotto2Tab;
  window.lotto2Predict = lotto2Predict;
  window.lotto2NavDraw = lotto2NavDraw;
  window.lotto2SelectDraw = lotto2SelectDraw;
  window.lotto2FetchAll = lotto2FetchAll;
  window.lotto2FetchLatest = lotto2FetchLatest;
  window.toggleLotto2RankDropdown = toggleLotto2RankDropdown;
  // 동적 HTML 인라인 onclick — 전역에 반드시 등록 (IIFE 내부 함수는 onclick에서 보이지 않음)
  window.lotto2SwitchBrainTab = lotto2SwitchBrainTab;
  // 동적 HTML 인라인 onclick — IIFE 밖 전역에 반드시 등록 (1군 전역 함수와 이름 분리)
  window.lotto2OnEliteBrainToggle = lotto2OnEliteBrainToggle;
  window.lotto2CloseTierWinsModal = lotto2CloseTierWinsModal;
  window.lotto2OpenTierWinsModal = lotto2OpenTierWinsModal;
  window.lotto2TierWinsModalNav = lotto2TierWinsModalNav;
  window.lotto2TierWinsModalGoDraw = lotto2TierWinsModalGoDraw;
  window.lotto2CopyKakaoText = lotto2CopyKakaoText;
  window.lotto2FameFilterRank = fameFilterRank;
  window.lotto2FameShowMore = fameShowMore;
  window.lotto2RankShowMore = lottoRankShowMore;
})();
