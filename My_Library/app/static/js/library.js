...
function platformIcon(platform) {
  ...
  if ((k === "youtube" || k === "유튜브") && typeof getYoutubeBrandMarkHtml === "function") {
    return getYoutubeBrandMarkHtml("yt-brand-icon");
  }
...
function renderCategoryTree(categories) {
  ...
  if (!catsVisible || catsVisible.length === 0) {
    ...
    bindMtFilters();
    bindTreeAll();
    if (typeof injectTreeFavicons === "function") injectTreeFavicons();
    return;
  }
  ...
  container.innerHTML = mtFilterHtml + treeAllHtml + catsVisible
    .map((cat) => {
      ...
      const treeLargeTitle =
        (cat.name && String(cat.name).trim()) ||
        (plat && plat !== "unknown" ? plat : "분류");
    ...
    <div class="tree-large" data-large="${escapeHtml(cat.name)}">
      <button type="button" class="tree-toggle tree-toggle-large brand-item ${largeOpen ? "open" : ""} ${largeOnlySel ? "active" : ""}" data-large="${escapeHtml(cat.name)}" data-medium="" data-small="" title="${escapeAttr(treeLargeTitle)}" aria-expanded="${largeOpen ? "true" : "false"}">
