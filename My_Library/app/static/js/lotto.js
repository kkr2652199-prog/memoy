...
let _lottoDetailDrawNo = null;
let _lottoDetailRows = null;
let _lottoPredFetchSeq = 0;
let _currentBrainTab = 'hyena';

/** 역대 1·2·3등 조건 필터용 brain_tag 집합(null=미로드). */
let _brainEliteTagSet = null;

/** 대시보드 brain_power → 태그별 맵(null=미구축). */
let _lottoBrainPowerByTag = null;
...
function setLottoBrainPowerCache(brainPower) {
  ...
  _lottoBrainPowerByTag = map;
}

async function ensureLottoBrainPowerLoaded() {
  if (_lottoBrainPowerByTag) return;
  try {
    const res = await fetch(_lottoResolveApiUrl('/api/lotto/dashboard-summary'));
    if (!res.ok) return;
...
const BRAIN_LIST = [
  { tag: 'stat', name: '통계', icon: '📊', color: '#3b82f6' },
  { tag: 'markov', name: '마르코프', icon: '🔗', color: '#10b981' },
  { tag: 'llm', name: 'LLM', icon: '🤖', color: '#f59e0b' },
  { tag: 'lstm', name: 'LSTM', icon: '🧠', color: '#ef4444' },
  { tag: 'fusion', name: '퓨전', icon: '⚡', color: '#8b5cf6' },
  { tag: 'hyena', name: '하이에나', icon: '🦁', color: '#eab308' },
  { tag: 'lead1', name: '1등가자', icon: '🎯', color: '#f97316' },
];
...
async function loadLottoDrawList() {
  try {
    const r = await fetch(resolveApiUrl('/api/lotto/predictions?limit=20000'));
    ...
    const dr = await fetch(resolveApiUrl('/api/lotto/draws?limit=10000'));
    ...
    const sel = document.getElementById('lottoDrawSelect');
    if (sel) {
...
function lottoSelectDraw(drawNo) {
...
async function lottoLoadSavedPrediction(drawNo, options) {
  ...
  if (fromTab && _lottoDetailDrawNo === d && Array.isArray(_lottoDetailRows) && _lottoDetailRows.length) {
    await renderPredictionsByBrain(d, _lottoDetailRows);
    return;
  }
  const hasRichUi = !!container.querySelector('.lotto-brain-tabs, .lotto-result-header');
  const useSoftShell = !!(softLoading && hasRichUi);
  const seq = ++_lottoPredFetchSeq;
  if (useSoftShell) {
    container.classList.add('lotto1-results-pending');
    container.setAttribute('aria-busy', 'true');
  } else {
    container.classList.remove('lotto1-results-pending');
    container.removeAttribute('aria-busy');
    container.innerHTML = '<p style="color: #888;">로딩 중...</p>';
  }
  try {
    const r = await fetch(resolveApiUrl('/api/lotto/predictions/draw/' + d));
    const data = await r.json();
    if (seq !== _lottoPredFetchSeq) {
      return;
    }
    const rows = (data && data.predictions) ? data.predictions : [];
    if (!rows.length) {
      _lottoDetailDrawNo = null;
      _lottoDetailRows = null;
      container.classList.remove('lotto1-results-pending');
      container.removeAttribute('aria-busy');
      container.innerHTML = `<p style="color: #888;">${d}회차 저장된 예측 없음. \"두뇌 예측\" 버튼으로 실행하세요.</p>`;
      return;
    }
    _lottoDetailDrawNo = d;
    _lottoDetailRows = rows;
...
function lottoSwitchBrainTab(brainTag) {
  _currentBrainTab = String(brainTag || '').toLowerCase();
  const drawNo = parseInt(document.getElementById('predictDrawNo').value, 10);
  if (drawNo) {
...
async function ensureBrainEliteTagsLoaded() {
  ...
  try {
    const r = await fetch(_lottoResolveApiUrl('/api/lotto/brain/elite-tags'));
    if (!r.ok) {
...
async function lottoOnEliteBrainToggle() {
  _brainEliteTagSet = null;
  const d = _lottoDetailDrawNo;
  const rows = _lottoDetailRows;
...
async function renderPredictionsByBrain(drawNo, rows) {
  ...
  const eliteCb = document.getElementById('lottoEliteBrainToggle');
  ...
  if (!byBrain[_currentBrainTab]) {
    ...
    _currentBrainTab = firstTag || (brainListForTabs[0] && brainListForTabs[0].tag) || 'hyena';
  }
  ...
  const tabsHtml = brainListForTabs.map((b) => {
    ...
    return `
      <button class="lotto-brain-tab ${active} ${disabled}"
              ...
              onclick="lottoSwitchBrainTab('${b.tag}')"
              ${disabled ? 'disabled' : ''}>
...
/** 세트 순번(#1~#5) → 카톡용 이모지 */
const _LOTTO_KAKAO_SET_EMOJI = ['', '🥇', '🥈', '🥉', '4️⃣', '5️⃣'];
...
/**
 * 1군 두뇌예측: 현재 불러온 예측(_lottoDetailRows)을 카카오톡용 텍스트로 복사.
 * 고적중 필터·뇌 탭 목록과 동일한 뇌 순서(BRAIN_LIST)로 전 뇌 출력(화면은 한 탭만 보여도 전 뇌 포함).
 */
async function lottoCopyKakaoText() {
  const rows = _lottoDetailRows;
  const drawNo = _lottoDetailDrawNo;
  ...
  brainListForTabs.forEach((b) => {
    ...
    list.forEach((row, i) => {
      const idx = i + 1;
      const emoji = _LOTTO_KAKAO_SET_EMOJI[idx] || '📌';
  ...
  try {
    ...
    console.warn('lottoCopyKakaoText', e);
    alert('복사에 실패했습니다. 브라우저 권한·HTTPS를 확인하세요.');
  }
}

/** 1~5등 적중 요약 모달 — 읽기 전용 API 연동 */
let _lottoTierWinsEscHandler = null;
...
function lottoCloseTierWinsModal() {
  ...
  if (_lottoTierWinsEscHandler) {
    document.removeEventListener('keydown', _lottoTierWinsEscHandler);
    _lottoTierWinsEscHandler = null;
  }
}
...
async function lottoRefreshTierWinsModalContent() {
  ...
  try {
    const r = await fetch(_lottoResolveApiUrl(`/api/lotto/predictions/draw/${d}/tier-wins`));
...
async function lottoOpenTierWinsModal() {
  const input = document.getElementById('predictDrawNo');
  const jump = document.getElementById('lottoTierWinsJumpInput');
  const d = input ? parseInt(input.value, 10) : NaN;
  if (!Number.isFinite(d) || d < 1) {
  ...
  _lottoTierWinsEscHandler = (e) => {
    if (e.key === 'Escape') lottoCloseTierWinsModal();
  };
  document.addEventListener('keydown', _lottoTierWinsEscHandler);
}
...
async function loadDashboard() {
  try {
    const res = await fetch('/api/lotto/dashboard-summary');
    const data = await res.json();
    setLottoBrainPowerCache(data.brain_power);
...
function renderRankings(rankings) {
  if (!rankings) return;
  window._lottoDashboardRankings = rankings;
  if (!window._lottoRankState) {
    window._lottoRankState = { rank1: { limit: 20 }, rank2: { limit: 20 }, rank3: { limit: 20 } };
  }
...
function lottoRankShowMore(key) {
  if (!window._lottoRankState) window._lottoRankState = {};
  const state = window._lottoRankState[key] || { limit: 20 };
  state.limit = Number(state.limit || 20) + 20;
  window._lottoRankState[key] = state;
  if (window._lottoDashboardRankings) {
    renderRankings(window._lottoDashboardRankings);
...
function lottoGetActiveStatusEl() {
  ...
  if (!activePage) return document.getElementById('lottoActionStatus');
  if (activePage.id === 'lotto-page-collect') {
    return document.getElementById('lottoCollectStatus') || document.getElementById('lottoActionStatus');
  }
  return document.getElementById('lottoActionStatus');
}
...
async function loadSpecialForce() {
  try {
    ...
    const predRes = await fetch(resolveApiUrl('/api/lotto/predictions?limit=50000'));
...
function renderSpecialScorecard(allPreds) {
  ...
  html += '<div style="font-weight:bold;color:#e0e0ff;margin-bottom:6px;">' + getBrainDisplayName('miss_analysis') + '</div>';
  ...
  html += '</div>';
  html += '<div style="border:1px solid #444;border-radius:8px;padding:10px;background:#0d1117;">';
  html += '<div style="font-weight:bold;color:#e0e0ff;margin-bottom:6px;">' + getBrainDisplayName('snake') + '</div>';
...
function _specialRenderRangeBars(arr, tag) {
  ...
  var title = getBrainDisplayName(tag);
...
function _specialRenderHitDetailRow(p) {
  ...
  html += '<span style="color:#8b9cf7;">' + getBrainDisplayName(tag) + '</span>';
  html += '</div>';
