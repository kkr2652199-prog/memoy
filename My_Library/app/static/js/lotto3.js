...
const BRAIN_DESCRIPTIONS = {
  ...
  v12_contrarian: '1·2군 회피 + D그룹 페어 사냥',
  v12_lstm: '시계열 딥러닝 AI',
  v12_fusion: '두뇌 합의 AI',
  v12_hyena: '메타 최적화 AI',
...
/** 회차 상세(탭 전환 시 LIMIT 절단 캐시로 역발상가 0건처럼 보이는 것 방지) */
let _lotto3DetailDrawNo = null;
let _lotto3DetailRows = null;
/** ◀▶ 빠른 연속 클릭 시 늦게 도착한 응답이 이전 화면을 덮지 않도록 */
let _lotto3PredFetchSeq = 0;
...
const BRAIN_LIST = [
  ...
  { tag: 'v12_contrarian', name: '역발상가', icon: '🦅', color: '#dc2626' },
...
/** 대시보드 brain_power → 태그별 맵(3군 전용). */
let _lotto3BrainPowerByTag = null;

function setLotto3BrainPowerCache(brainPower) {
  const map = {};
  (brainPower || []).forEach((b) => {
    const k = String(b.brain || '').toLowerCase();
    if (k) map[k] = b;
  });
  _lotto3BrainPowerByTag = map;
}

async function ensureLotto3BrainPowerLoaded() {
  if (_lotto3BrainPowerByTag) return;
  try {
    const res = await fetch(_lottoResolveApiUrl('/api/lotto3/v12/dashboard-summary'));
    if (!res.ok) return;
    const data = await res.json();
    setLotto3BrainPowerCache(data.brain_power);
  } catch (e) {
    console.warn('lotto3 brain_power cache:', e);
  }
}

function lotto3BrainTierNanoHtml(tag) {
  const m = _lotto3BrainPowerByTag || {};
...
function lotto3KakaoBrainRecordLine(tag) {
  const m = _lotto3BrainPowerByTag || {};
  const b = m[String(tag).toLowerCase()] || {};
  const n1 = Number(b.rank1 || 0);
  const n2 = Number(b.rank2 || 0);
...
const _LOTTO3_KAKAO_SET_EMOJI = ['', '🥇', '🥈', '🥉', '4️⃣', '5️⃣'];
...
/** V12 POST 오류 reason/error 문자열 → 사용자용 문구 (2군과 동일 계열) */
function lotto3V12ErrorReasonText(reason) {
...
async function loadLottoDrawList() {
  try {
    ...
    const dr = await fetch(resolveApiUrl('/api/lotto3/v12/draws?limit=10000'));
...
async function lottoLoadSavedPrediction(drawNo, options) {
  ...
  if (fromTab && _lotto3DetailDrawNo === d && Array.isArray(_lotto3DetailRows) && _lotto3DetailRows.length) {
    await renderPredictionsByBrain(d, _lotto3DetailRows);
    return;
  }
  const hasRichUi = !!container.querySelector('.lotto-brain-tabs, .lotto-result-header');
  const useSoftShell = !!(softLoading && hasRichUi);
  const seq = ++_lotto3PredFetchSeq;
  if (useSoftShell) {
    container.classList.add('lotto3-results-pending');
    container.setAttribute('aria-busy', 'true');
  } else {
    container.classList.remove('lotto3-results-pending');
    container.removeAttribute('aria-busy');
    container.innerHTML = '<p style="color: #888;">로딩 중...</p>';
  }
  try {
    const r = await fetch(resolveApiUrl('/api/lotto3/v12/predictions/draw/' + d));
    const data = await r.json();
    if (seq !== _lotto3PredFetchSeq) {
      return;
    }
    const rows = (data && data.predictions) ? data.predictions : [];
    if (!rows.length) {
      _lotto3DetailDrawNo = null;
      _lotto3DetailRows = null;
      container.classList.remove('lotto3-results-pending');
      container.removeAttribute('aria-busy');
      container.innerHTML = `<p style="color: #888;">${d}회차 저장된 예측 없음. \"두뇌 예측\" 버튼으로 실행하세요.</p>`;
      return;
    }
    _lotto3DetailDrawNo = d;
    _lotto3DetailRows = rows;
...
async function lotto3EnsureBrainEliteTagsLoaded() {
  if (_brainEliteTagSet !== null) {
    return true;
  }
  try {
    const r = await fetch(_lottoResolveApiUrl('/api/lotto3/v12/brain/elite-tags'));
    if (!r.ok) {
    ...
    console.warn('lotto3 brain elite-tags:', e);
    return false;
  }
}

async function lotto3OnEliteBrainToggle() {
  _brainEliteTagSet = null;
  const d = _lotto3DetailDrawNo;
  const rows = _lotto3DetailRows;
...
async function renderPredictionsByBrain(drawNo, rows) {
  const container = document.getElementById('lotto3PredictionResults');
  if (!container) return;
  await ensureLotto3BrainPowerLoaded();
  container.classList.remove('lotto3-results-pending');
  container.removeAttribute('aria-busy');
  ...
  const eliteCb = document.getElementById('lotto3EliteBrainToggle');
  const eliteOn = !!(eliteCb && eliteCb.checked);

  let brainListForTabs = BRAIN_LIST;
  if (eliteOn) {
    const ok = await lotto3EnsureBrainEliteTagsLoaded();
    if (ok) {
  ...
  if (!byBrain[_currentBrainTab]) {
    ...
    _currentBrainTab = firstTag || (brainListForTabs[0] && brainListForTabs[0].tag) || 'v12_hyena';
  }
  ...
  const tabsHtml = brainListForTabs.map((b) => {
    ...
    const nano = lotto3BrainTierNanoHtml(b.tag);
    return `
  ...
  container.innerHTML = `
    ...
    <div class="lotto-brain-cards" id="lotto3BrainCards">${cardsHtml}</div>
  `;
}

/**
 * 3군: 현재 불러온 예측(_lotto3DetailRows)을 카카오톡용 텍스트로 복사.
 * 고적중 필터·뇌 탭 목록과 동일한 뇌 순서(BRAIN_LIST)로 전 뇌 출력.
 */
async function lotto3CopyKakaoText() {
  const rows = _lotto3DetailRows;
  const drawNo = _lotto3DetailDrawNo;
  if (!rows || !rows.length || !drawNo) {
    alert('먼저 회차를 선택한 뒤 "3군 두뇌 예측"을 눌러 예측을 불러오세요.');
    return;
  }
  ...
  brainListForTabs.forEach((b) => {
    ...
    list.forEach((row, i) => {
      const idx = i + 1;
      const emoji = _LOTTO3_KAKAO_SET_EMOJI[idx] || '📌';
    ...
    body += lotto3KakaoBrainRecordLine(b.tag);
  });
  ...
  try {
    ...
    console.warn('lotto3CopyKakaoText', e);
    alert('복사에 실패했습니다. 브라우저 권한·HTTPS를 확인하세요.');
  }
}

/** 3군: 1~5등 적중 요약 모달 — 읽기 전용 API */
let _lotto3TierWinsEscHandler = null;

function lotto3CloseTierWinsModal() {
  const modal = document.getElementById('lotto3TierWinsModal');
  if (modal) {
    modal.hidden = true;
    modal.setAttribute('aria-hidden', 'true');
  }
  if (_lotto3TierWinsEscHandler) {
    document.removeEventListener('keydown', _lotto3TierWinsEscHandler);
    _lotto3TierWinsEscHandler = null;
  }
}

function lotto3TierWinsRankTitleClass(rank) {
...
function lotto3RenderTierWinsModalContent(data) {
  const body = document.getElementById('lotto3TierWinsModalBody');
  const title = document.getElementById('lotto3TierWinsModalTitle');
  ...
  for (let r = 1; r <= 5; r++) {
    ...
    html += `<section class="lotto-tier-wins-rank"><h3 class="${lotto3TierWinsRankTitleClass(r)}">${rankTitles[r]}</h3><ul class="lotto-tier-wins-list">`;
    list.forEach((it) => {
...
async function lotto3RefreshTierWinsModalContent() {
  const input = document.getElementById('lotto3PredictDrawNo');
  const d = input ? parseInt(input.value, 10) : NaN;
  const body = document.getElementById('lotto3TierWinsModalBody');
  const title = document.getElementById('lotto3TierWinsModalTitle');
  const jump = document.getElementById('lotto3TierWinsJumpInput');
  if (!body) return;
...
async function lotto3OpenTierWinsModal() {
  ...
  await lotto3RefreshTierWinsModalContent();
  _lotto3TierWinsEscHandler = (e) => {
    if (e.key === 'Escape') lotto3CloseTierWinsModal();
  };
  document.addEventListener('keydown', _lotto3TierWinsEscHandler);
}

async function lotto3TierWinsModalNav(delta) {
  if (!_lottoDrawList.length) {
    await lotto3RefreshTierWinsModalContent();
    return;
  }
...
async function lotto3TierWinsModalGoDraw() {
  const jump = document.getElementById('lotto3TierWinsJumpInput');
...
async function lotto3Predict() {
  ...
  try {
    ...
    if (data.error) {
      lottoSetActionStatusText(status, lotto3V12ErrorReasonText(data.error), '#f87171');
      return;
    }
    if (data.status === 'error') {
      lottoSetActionStatusText(status, lotto3V12ErrorReasonText(data.reason), '#f87171');
      return;
    }
    ...
    try {
      ...
      if (sel) {
        ...
        if (rawPreds.length > 0) {
          ...
          // cached·구버전: bulk 목록은 LIMIT+정렬로 뇌별 행이 잘릴 수 있음 → 회차 전용 조회
          await loadLottoDrawList();
          try {
            const pr = await fetch(resolveApiUrl('/api/lotto3/v12/predictions/draw/' + drawNo));
...
function renderHallOfFame() {
  ...
  // 6컬럼 그리드
  var brains = ['v12_stat', 'v12_run', 'v12_offset', 'v12_contrarian', 'v12_lstm', 'v12_fusion', 'v12_hyena', 'v12_snake'];
...
document.addEventListener('DOMContentLoaded', () => {
  const army3TabBtn = document.querySelector('[data-tab="lotto-army3"]');
  if (army3TabBtn) {
    army3TabBtn.addEventListener('click', () => {
      setTimeout(() => {
        loadLotto3Dashboard();
        loadLotto3BrainStatus();
...
function switchLotto3Page(pageName) {
  const root = document.getElementById('tab-lotto-army3');
  if (!root) return;
...
function renderProgress(range, totalPreds) {
  ...
  if (!el || !range) return;
  const total = Number(range.total_draws) || 1;
  const start = Number(range.start) || 0;
  const endRaw = Number(range.end) || 0;
  // API가 end를 넘겨도 당첨 건수(total)를 넘는 표시 방지 (이상 target_draw_no 대비)
  const endShown = Math.min(endRaw, total);
  const learned = Math.max(0, endShown - start + 1);
  const pct = Math.min(100, Math.round((learned / Math.max(total, 1)) * 100));
  el.innerHTML =
    '<div style="display:flex;justify-content:space-between;color:#fff;margin-bottom:8px">' +
      '<span>학습 범위: ' + start + ' ~ ' + endShown + '회차</span>' +
...
function lottoGetActiveStatusEl() {
  ...
  window.lotto3OnEliteBrainToggle = lotto3OnEliteBrainToggle;
  window.lotto3CloseTierWinsModal = lotto3CloseTierWinsModal;
  window.lotto3OpenTierWinsModal = lotto3OpenTierWinsModal;
  window.lotto3TierWinsModalNav = lotto3TierWinsModalNav;
  window.lotto3TierWinsModalGoDraw = lotto3TierWinsModalGoDraw;
  window.lotto3CopyKakaoText = lotto3CopyKakaoText;
