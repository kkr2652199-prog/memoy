...
FALLBACK_ORDER = ["openai", "claude", "gemini", "local", "lmstudio"]
KEY_FIELDS = {
...
def get_provider(provider_name: str, config: dict) -> BaseLLMClient:
    ...
    if provider_name == "openai":
        ...
        return LMStudioClient(
            endpoint=llm_cfg.get("lmstudio_endpoint", "http://localhost:1234"),
            model=llm_cfg.get("lmstudio_model", "gemma4:e2b"),
        )
    else:
...
def list_providers() -> list[dict]:
    return [
        ...
        {"id": "lmstudio", "name": "LM Studio (로컬)", "requires_key": False},
    ]
...
def normalize_chat_provider_id(raw: str | None) -> str | None:
    ...
    if not s or s in ("default", "auto"):
        return None
    allowed = {"openai", "claude", "gemini", "local", "lmstudio"}
...
def build_chat_provider_candidates(config: dict, preferred: str | None) -> list[str]:
    ...
    def add(name: str) -> None:
        if name in seen:
            return
        if name not in ("openai", "claude", "gemini", "local", "lmstudio"):
            return
        if name == "local" or name == "lmstudio":
            seen.add(name)
            out.append(name)
            return
