...
def _army1_brain_set_count(conn, draw_no: int, brain_tag: str) -> int:
    if brain_tag == "llm":
        row = conn.execute(
            ...
            "AND brain_tag IN ('llm','llm_fallback')",
            (draw_no,),
        ).fetchone()
    else:
        row = conn.execute(
...
def _army1_predictions_ready(conn, draw_no: int) -> bool:
    ...
    return int(lead1 or 0) >= _ARMY1_MIN_SETS


def maybe_generate_army1_next_predictions(scored_draw_no: int) -> dict:
    """당첨 확정 회차 N 처리 후 N+1 1군 예측 자동 생성.

    - N회차 당첨 확정(num1) 필수
    - run_prediction(N+1): _get_draws_before(N+1) → 1~N만 사용 (컨닝 방지)
    - lead1: save_brain7_predictions (5뇌 READ-ONLY)
    - 멱등: 6뇌+lead1 이미 5세트씩 있으면 skip
    - 6뇌 예측 알고리즘 코드 변경 없음 (기존 run_prediction 호출만)
    """
    next_no = scored_draw_no + 1
    conn = get_lotto_db()
    try:
        row = conn.execute(
            "SELECT num1 FROM lotto_draws WHERE draw_no = ?", (scored_draw_no,),
        ).fetchone()
        if not row or row[0] is None:
            return {
                "generated": False,
                "reason": "scored_draw_not_confirmed",
                "scored_draw_no": scored_draw_no,
            }

        if _army1_predictions_ready(conn, next_no):
            return {
                "generated": False,
                "reason": "already_exists",
                "target_draw_no": next_no,
            }

        draws = _get_draws_before(next_no)
        if not draws:
            return {
                "generated": False,
                "reason": "no_prior_draws",
                "target_draw_no": next_no,
            }
        max_prior = max(d["draw_no"] for d in draws)
        if max_prior >= next_no:
            return {
                "generated": False,
    ...
    if need_six:
        ...
        if "error" in pred:
            return {
                ...
                "reason": pred["error"],
                "target_draw_no": next_no,
                "max_draw_no_data": max_prior,
            }
        six_result = {"total_sets": pred.get("total_sets"), "status": pred.get("status")}

    lead1_n = 0
    if lead1_cnt < _ARMY1_MIN_SETS:
        conn3 = get_lotto_db()
        try:
            lead1_n = save_brain7_predictions(conn3, next_no)
            conn3.commit()
        finally:
            conn3.close()

    logger.info(
        "[army1_auto] next predictions draw=%d (after scored=%d) six=%s lead1=%d max_data=%d",
        next_no, scored_draw_no, six_result, lead1_n, max_prior,
    )
    return {
        "generated": True,
        "scored_draw_no": scored_draw_no,
        "target_draw_no": next_no,
        "max_draw_no_data": max_prior,
        "six_brain": six_result,
        "lead1_sets": lead1_n if lead1_n else lead1_cnt,
    }


# 2군 V11 자동 예측 (7뇌×5 = 35세트, v11_*)
_ARMY2_MIN_V11_SETS = 35


def _army2_v11_set_count(conn, draw_no: int) -> int:
    row = conn.execute(
        "SELECT COUNT(*) FROM lotto_predictions_army2 "
        "WHERE target_draw_no=? AND brain_tag LIKE 'v11_%'",
        (draw_no,),
    ).fetchone()
    return int(row[0] or 0)


def _army2_v11_predictions_ready(conn, draw_no: int) -> bool:
    return _army2_v11_set_count(conn, draw_no) >= _ARMY2_MIN_V11_SETS


def maybe_generate_army2_next_predictions(scored_draw_no: int) -> dict:
    """당첨 확정 회차 N 처리 후 N+1 2군(V11) 예측 자동 생성.

    - get_v11_training_draws(N+1) → draw_no < N+1 (컨닝 방지)
    - run_prediction_v11(N+1): 6뇌+snake 35세트
    - 멱등: v11_* 35세트 이상 있으면 skip
    - v11 예측 알고리즘 코드 변경 없음
    """
    next_no = scored_draw_no + 1
    conn = get_lotto_db()
    try:
        ...
        if _army2_v11_predictions_ready(conn, next_no):
            return {
                "generated": False,
                "reason": "already_exists",
                "target_draw_no": next_no,
                "v11_sets": _army2_v11_set_count(conn, next_no),
            }

        from app.lotto2.v11_models import get_v11_training_draws

        training = get_v11_training_draws(next_no)
        if len(training) < 5:
            return {
                "generated": False,
        ...
        max_prior = max(int(d["draw_no"]) for d in training)
        if max_prior >= next_no:
            return {
                "generated": False,
                "reason": "contamination",
    ...
    result = run_prediction_v11(next_no)
    status = result.get("status", "error")
    v11_sets = int(result.get("v11_sets") or 0)

    if status not in ("ok", "cached") or v11_sets < _ARMY2_MIN_V11_SETS:
        return {
            "generated": False,
    ...
    logger.info(
        "[army2_auto] next predictions draw=%d (after scored=%d) v11_sets=%d max_data=%d",
        next_no, scored_draw_no, v11_sets, max_prior,
    )
    return {
        "generated": True,
        "scored_draw_no": scored_draw_no,
...
# 3군 V12 자동 예측 (8뇌×5 = 40세트, v12_*)
_ARMY3_MIN_V12_SETS = 40


def _army3_v12_set_count(conn, draw_no: int) -> int:
    row = conn.execute(
        "SELECT COUNT(*) FROM lotto_predictions_army3 "
        "WHERE target_draw_no=? AND brain_tag LIKE 'v12_%'",
        (draw_no,),
    ).fetchone()
    return int(row[0] or 0)


def _army3_v12_predictions_ready(conn, draw_no: int) -> bool:
    return _army3_v12_set_count(conn, draw_no) >= _ARMY3_MIN_V12_SETS


def maybe_generate_army3_next_predictions(scored_draw_no: int) -> dict:
    """당첨 확정 회차 N 처리 후 N+1 3군(V12) 예측 자동 생성.

    - get_v12_training_draws(N+1) → draw_no < N+1 (컨닝 방지)
    - run_prediction_v12(N+1): 8뇌 40세트 (DZ 필터·win_avoid 포함)
    - 멱등: v12_* 40세트 이상 있으면 skip
    - v12 예측 알고리즘 코드 변경 없음
    """
    next_no = scored_draw_no + 1
    conn = get_lotto_db()
    try:
        ...
        if _army3_v12_predictions_ready(conn, next_no):
            return {
                "generated": False,
                "reason": "already_exists",
                "target_draw_no": next_no,
                "v12_sets": _army3_v12_set_count(conn, next_no),
            }

        from app.lotto3.v12_models import get_v12_training_draws

        training = get_v12_training_draws(next_no)
        if len(training) < 5:
            return {
                "generated": False,
                "reason": "insufficient_training_data",
    ...
    result = run_prediction_v12(next_no)
    status = result.get("status", "error")
    v12_sets = int(result.get("v12_sets") or 0)

    if status not in ("ok", "cached") or v12_sets < _ARMY3_MIN_V12_SETS:
        return {
            "generated": False,
            "reason": result.get("reason") or status,
            "target_draw_no": next_no,
    ...
    logger.info(
        "[army3_auto] next predictions draw=%d (after scored=%d) v12_sets=%d max_data=%d",
        next_no, scored_draw_no, v12_sets, max_prior,
    )
    return {
        "generated": True,
        "scored_draw_no": scored_draw_no,
...
def refresh_all_army_prediction_scores(target_draw_no: int) -> None:
    """1·2·3군: `lotto_draws`에 당첨이 있으면 해당 회차 예측 재채점."""
    from app.lotto.engine import refresh_prediction_scores_for_target_draw
    from app.lotto2.v11_engine import refresh_v11_prediction_scores_for_target_draw
    from app.lotto3.v12_engine import refresh_v12_prediction_scores_for_target_draw
    ...
    refresh_v11_prediction_scores_for_target_draw(target_draw_no)
    refresh_v12_prediction_scores_for_target_draw(target_draw_no)
    # 1군 Layer3: 당첨 확정 후 brain_weights 자동 갱신 (6뇌 예측값 READ-ONLY)
    maybe_update_brain_weights_after_scoring(target_draw_no)
    # 2군 V11: 당첨 확정 후 v11 가중치 자동 갱신
    from app.lotto2.v11_models import maybe_update_v11_weights_after_scoring

    maybe_update_v11_weights_after_scoring(target_draw_no)
    # 3군 V12: 당첨 확정 후 v12 가중치 자동 갱신
...
def backfill_unscored_army2_army3_predictions() -> dict:
    """당첨 DB는 있는데 matched_count<0 인 2·3군 예측 일괄 재채점(기동·수집 후 보정)."""
    from app.lotto2.v11_engine import refresh_v11_prediction_scores_for_target_draw
    from app.lotto3.v12_engine import refresh_v12_prediction_scores_for_target_draw
    ...
    try:
        army2_draws = [
            ...
            for r in conn.execute(
                ...
                WHERE p.matched_count < 0 AND p.brain_tag LIKE 'v11_%'
        ...
        army3_draws = [
            ...
            for r in conn.execute(
                ...
                INNER JOIN lotto_draws d ON d.draw_no = p.target_draw_no
                WHERE p.matched_count < 0 AND p.brain_tag LIKE 'v12_%'
                ORDER BY p.target_draw_no
                """
            ).fetchall()
        ]
    ...
    for d in army2_draws:
        refresh_v11_prediction_scores_for_target_draw(d)
    for d in army3_draws:
        refresh_v12_prediction_scores_for_target_draw(d)

    out = {
        "army2_draws": len(army2_draws),
        "army3_draws": len(army3_draws),
        "army2_rescored": len(army2_draws),
        "army3_rescored": len(army3_draws),
    }
    if army2_draws or army3_draws:
        logger.info("2·3군 미채점 재채점 완료: %s", out)
    return out
...
def fetch_all_draws(delay: float = 0.3) -> dict:
    ...
    try:
        while True:
            ...
            save_draw(draw)
            refresh_all_army_prediction_scores(draw["draw_no"])
            fetched += 1
            draw_no += 1
