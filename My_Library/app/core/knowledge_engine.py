...
중요한 규칙:
- 같은 주제를 다른 관점/방법으로 다룬 것은 모순이 아니다. 스킵해.
- 프로젝트 방향 전환이나 도구 변경은 "supersession"이다.
- 가격, 정책, 버전 등 시간 경과로 달라진 것은 "supersession"이다.
- 같은 시점에 논리적으로 양립 불가능한 주장만 "contradiction"이다.
- 모순이 없으면 반드시 빈 배열 []만 반환해. 억지로 찾지 마.
- description에 "모순이 없음" 같은 문장을 넣지 마. 없으면 그냥 []를 반환해.

모순 유형:
1. "contradiction" — 같은 시점에 논리적으로 양립 불가능한 주장
2. "supersession" — 새 자료가 기존 정보를 업데이트/대체
...
JSON 배열로 반환해. 없으면 빈 배열 []:
[{{"material_id": 기존자료ID, "type": "contradiction" 또는 "supersession", "description": "구체적 설명"}}]
반드시 JSON 배열만 반환해."""
...
JUDGE_PROMPT = """이 두 자료 사이의 관계를 판단해줘.

자료 A (최신): {new_title}
요약: {new_summary}

자료 B (기존): {existing_title}
요약: {existing_summary}

기존 모순 설명: {description}

아래 4가지 중 하나로 분류하고, 이유를 한 줄로 설명해:
1. "temporal" — 시간 경과로 정보가 업데이트됨 (A가 B를 대체)
2. "scope" — 범위/관점이 달라서 다르게 보이는 것 (모순 아님)
3. "genuine" — 같은 시점에 논리적으로 양립 불가능한 주장
4. "none" — 모순이 아님 (오탐)

JSON으로 반환: {{"type": "temporal|scope|genuine|none", "reason": "한줄 설명"}}
반드시 JSON만 반환해."""


async def judge_contradiction(contradiction, db) -> dict:
    """LLM을 사용해 모순을 4가지 타입으로 자동 분류하고 처리한다.

    1티어 패턴(ktrysmt/llmwiki v0.6) 기반:
    - temporal: 시간 경과 → auto_resolved (최신이 승리)
    - scope: 범위/관점 차이 → auto_resolved
    - none: 오탐 → auto_resolved
    - genuine: 진짜 모순 → suggested 유지 (사람 확인 필요)
    """
    from app.db.models import Material, Notification

    mat_new = db.query(Material).filter(Material.id == contradiction.material_id_new).first()
    mat_existing = db.query(Material).filter(Material.id == contradiction.material_id_existing).first()

    if not mat_new or not mat_existing:
        contradiction.status = "auto_resolved"
        contradiction.resolved_at = datetime.now(timezone.utc)
        contradiction.resolution_note = "Lint Judge: 자료가 삭제되어 자동 해결"
        db.add(contradiction)
        return {"type": "none", "reason": "자료 누락"}

    prompt = JUDGE_PROMPT.format(
        new_title=mat_new.title,
        new_summary=(mat_new.summary or "")[:300],
        existing_title=mat_existing.title,
        existing_summary=(mat_existing.summary or "")[:300],
        description=(contradiction.description or "")[:200],
    )

    raw = await _llm_call(prompt)
    if not raw:
        contradiction.status = "suggested"
        contradiction.resolution_note = "Lint Judge: LLM 응답 없음, 수동 확인"
        db.add(contradiction)
        return {"type": "error", "reason": "LLM 응답 없음"}

    try:
        cleaned = re.sub(r'```json\s*', '', raw)
        cleaned = re.sub(r'```\s*$', '', cleaned).strip()
        result = json.loads(cleaned)
        judge_type = result.get("type", "none")
        reason = result.get("reason", "")
    except (json.JSONDecodeError, KeyError, AttributeError):
        judge_type = "none"
        reason = f"JSON 파싱 실패, 원본: {raw[:100]}"

    now = datetime.now(timezone.utc)

    if judge_type == "temporal":
        contradiction.status = "auto_resolved"
        contradiction.contradiction_type = "supersession"
        contradiction.resolved_at = now
        contradiction.resolution_note = f"Lint Judge(temporal): {reason}"
        from app.core.health_check import _material_tags_add
        _material_tags_add(mat_existing, "superseded")
        db.add(mat_existing)

    elif judge_type == "scope":
        contradiction.status = "auto_resolved"
        contradiction.resolved_at = now
        contradiction.resolution_note = f"Lint Judge(scope): {reason}"

    elif judge_type == "none":
        contradiction.status = "auto_resolved"
        contradiction.resolved_at = now
        contradiction.resolution_note = f"Lint Judge(none): {reason}"

    elif judge_type == "genuine":
        contradiction.status = "suggested"
        contradiction.resolution_note = f"Lint Judge(genuine): {reason} — 사람 확인 필요"
        db.add(Notification(
            type="contradiction",
            message=f"⚠️ 진짜 모순 확인됨: '{mat_new.title}' vs '{mat_existing.title}' — {reason[:80]}",
            related_material_id=mat_new.id,
        ))
    else:
        contradiction.status = "auto_resolved"
        contradiction.resolved_at = now
        contradiction.resolution_note = f"Lint Judge(unknown→none): {reason}"

    db.add(contradiction)
    return {"type": judge_type, "reason": reason}
