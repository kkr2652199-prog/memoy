...
핵심태그 '{entity_name}' ({entity_type})의 위키 개요를 갱신하라.
...
[규칙]
1. 첫 문장은 이 핵심태그만의 고유 정의. "다양한 AI 도구 중 하나" 같은 일반 문구 금지.
2. 핵심 특징은 최소 3개, 최대 5개. 이 핵심태그만의 차별점.
3. 기존 내용에 사실 오류가 있으면 수정하라. 기존 내용을 맹목적으로 복사하지 마라.
4. [[링크]]는 실제 핵심태그/주제만. Q&A, 자료 제목은 링크하지 마라.
5. [자료 ID:숫자]에는 반드시 숫자만 넣어라. 날짜를 넣지 마라.
...
def _validate_wiki_overview(text: str, entity_name: str) -> bool:
    valid, reason = validate_wiki_text(text, entity_name)
    if not valid:
        logger.warning("위키 품질 검증 실패: %s — %s", entity_name, reason)
    return valid
...
def _write_entity_wiki_page_from_overview(
    ...
    if not _validate_wiki_overview(overview, entity_name):
        return
...
async def _entity_overview_single_llm(
    ...
    if llm_result:
        ...
        if _validate_wiki_overview(cand, entity_name):
            return cand
...
async def update_entity_page(
    ...
    if llm_result:
        ...
        if not _validate_wiki_overview(overview, entity_name):
            if "## 개요" in existing_content:
                st = existing_content.index("## 개요") + len("## 개요")
