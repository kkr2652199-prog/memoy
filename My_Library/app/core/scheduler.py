...
def _lotto_weekly_fetch_latest():
    """주 1회 최신 로또 회차를 자동 수집한다."""
    try:
        from app.lotto.data_service import fetch_latest_draw

        draw = fetch_latest_draw()
        if draw:
            logger.info("로또 최신 회차 자동 수집 완료: %s회", draw.get("draw_no"))
        else:
            logger.info("로또 최신 회차 자동 수집: 신규 회차 없음(또는 아직 미공개)")
    except Exception as e:
        logger.exception("로또 최신 회차 자동 수집 실패: %s", e)
...
def start_scheduler(interval_hours: int = 24):
    if not scheduler.running:
        ...
        # 서버 기동 후 1회 즉시 확인(자동 갱신 체감용)
        scheduler.add_job(
            _lotto_weekly_fetch_latest,
            "date",
            id="lotto_fetch_latest_once_on_startup",
            replace_existing=True,
        )
        # 로또 최신 회차 자동 갱신: 매주 일요일 00:30 (토요일 추첨 이후)
        scheduler.add_job(
            _lotto_weekly_fetch_latest,
            CronTrigger(day_of_week="sun", hour=0, minute=30),
            id="lotto_fetch_latest_weekly",
            replace_existing=True,
        )
        scheduler.start()
