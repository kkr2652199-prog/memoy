...
DOMAIN_BRAND_MAP: dict[str, tuple[str, str]] = {
    ...
    # MBC iM뉴스 등 (imnews.imbc.com) — mbc.co.kr와 호스트가 달라 별도 키 필요
    "imbc.com": ("MBC 뉴스", "news_broadcast"),
    ...
    "newsis.com": ("뉴시스", "뉴스"),
    "yna.co.kr": ("연합뉴스", "뉴스"),
    "yonhapnews.co.kr": ("연합뉴스", "뉴스"),
    "mk.co.kr": ("매일경제", "뉴스"),
    # 매일신문(www.imaeil.com) — mk.co.kr(매일경제)과 도메인이 다름
    "imaeil.com": ("매일신문", "뉴스"),
    "mt.co.kr": ("머니투데이", "뉴스"),
    "hankyung.com": ("한국경제", "뉴스"),
    "sedaily.com": ("서울경제", "뉴스"),
    "chosun.com": ("조선일보", "뉴스"),
    "donga.com": ("동아일보", "뉴스"),
    "hani.co.kr": ("한겨레", "뉴스"),
    "khan.co.kr": ("경향신문", "뉴스"),
    "joongang.co.kr": ("중앙일보", "뉴스"),
    "joins.com": ("중앙일보", "뉴스"),
    "biz.chosun.com": ("조선비즈", "뉴스"),
    "zdnet.co.kr": ("ZDNet", "뉴스"),
    "bloter.net": ("블로터", "뉴스"),
    "etnews.com": ("전자신문", "뉴스"),
    "edaily.co.kr": ("이데일리", "뉴스"),
    "fnnews.com": ("파이낸셜뉴스", "뉴스"),
    "asiae.co.kr": ("아시아경제", "뉴스"),
    "herald.co.kr": ("헤럴드경제", "뉴스"),
    "hankookilbo.com": ("한국일보", "뉴스"),
    "kmib.co.kr": ("국민일보", "뉴스"),
    "segye.com": ("세계일보", "뉴스"),
    "reuters.com": ("Reuters", "뉴스"),
    "bloomberg.com": ("Bloomberg", "뉴스"),
...
_DOMAIN_LABELS_FALLBACK: dict[str, tuple[str, str]] = {
    "news.naver.com": ("뉴스", "네이버뉴스"),
    "n.news.naver.com": ("뉴스", "네이버뉴스"),
    "v.daum.net": ("뉴스", "다음뉴스"),
    "news.daum.net": ("뉴스", "다음뉴스"),
    "biz.chosun.com": ("뉴스", "조선비즈"),
    "www.chosun.com": ("뉴스", "조선일보"),
    "www.hani.co.kr": ("뉴스", "한겨레"),
    "www.donga.com": ("뉴스", "동아일보"),
    "wowtv.co.kr": ("news_broadcast", "한국경제TV"),
    "www.wowtv.co.kr": ("news_broadcast", "한국경제TV"),
    "www.hankyung.com": ("뉴스", "한국경제"),
    "www.mk.co.kr": ("뉴스", "매일경제"),
    "www.sedaily.com": ("뉴스", "서울경제"),
    "www.edaily.co.kr": ("뉴스", "이데일리"),
    # 포탈
    ...
    # Google AI (google.com·www.google.com보다 먼저 시도 — _lookup_domain_labels_fallback 정렬)
    "notebooklm.google.com": ("ai_service", "노트북LM"),
    # AI 서비스
    "ollama.com": ("ai_service", "Ollama"),
    "www.ollama.com": ("ai_service", "Ollama"),
    ...
    # 이미지 웹앱·로컬 도구
    "iloveimg.com": ("devtools", "iLoveIMG"),
    "www.iloveimg.com": ("devtools", "iLoveIMG"),
    "squoosh.app": ("devtools", "Squoosh"),
    "www.squoosh.app": ("devtools", "Squoosh"),
    "obsidian.md": ("devtools", "Obsidian"),
    "www.obsidian.md": ("devtools", "Obsidian"),
}
...
def _lookup_domain_labels_fallback(netloc: str) -> tuple[str, str] | None:
    """google.com보다 notebooklm.google.com 같이 구체적인 호스트를 먼저 매칭."""
    try:
        items = list(_DOMAIN_LABELS_FALLBACK.items())
        items.sort(
            key=lambda kv: len(_normalize_host(kv[0])),
            reverse=True,
        )
...
def _fetch_html_for_brand(url: str) -> str:
    ...
    try:
        ...
        if resp.status_code == 200:
            return decode_http_response_text(resp) or ""
    except Exception:
        pass
    return ""
...
_KNOWN_KOREAN_PLATFORMS = frozenset({
    "유튜브", "뉴스", "블로그", "SNS", "포탈", "서비스", "문서", "직접입력", "기타",
    "AI서비스", "개발도구", "쇼핑", "여행", "교육", "금융", "커뮤니티",
    "엔터테인먼트", "음식/배달", "종합사이트",
})
...
def platform_code_to_korean(code: str) -> str:
    ...
    c = (code or "").strip()
    if c in _KNOWN_KOREAN_PLATFORMS:
        return c
    lut = {
        "youtube": "유튜브",
        "news_broadcast": "뉴스",
