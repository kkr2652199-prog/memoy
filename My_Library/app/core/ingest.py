...
from app.core.site_classifier import classify_site, get_homepage_url  # noqa: F401
...
from app.core.cross_references import (                  # noqa: F401
    ...
    find_cross_reference_targets_for_single,
...
def ingest_material(
    ...
    # [OFF] 중복 URL 체크 비활성화 — 잘못 섭취하면 삭제로 처리
    # if su and not force:
    #     existing = (
    #         db.query(Material)
    ...
    related = find_cross_reference_targets_for_single(
        db, material, material_type=material_type
    )
    if related:
    ...
    # 의미연결 + 엔티티연결 (새 자료 1건만 비교 — O(n). semantic은 임베딩 행이 있어야 함)
    try:
        from app.core.cross_references import (
            create_entity_based_cross_references_for_single,
            create_semantic_cross_references_for_single,
        )

        create_semantic_cross_references_for_single(
            db, material.id, similarity_threshold=0.75
        )
        create_entity_based_cross_references_for_single(db, material.id)
    except Exception as e:
        logger.warning("추가 교차참조 생성 중 오류 (무시): %s", e)
...
async def ingest_webpage(
    ...
    # 사이트 분류기로 1차 판단
    site_result = classify_site(url)
    site_category = ""
    site_name = ""
    if site_result:
        site_category, site_name = site_result
        # fetch 결과의 brand_info가 더 구체적이면 (예: 블로그 파서) 그걸 유지
        fetched_platform = (brand_info.get("platform") or "").strip()
        if fetched_platform and fetched_platform != "unknown" and fetched_platform != site_category:
            # fetch 단계에서 이미 구체적 platform을 설정한 경우 (예: 네이버블로그 → 블로그)
            logger.info(
                "fetch brand_info 우선: %s (site_classifier: %s)",
                fetched_platform,
                site_category,
            )
            site_category = fetched_platform
            site_name = brand_info.get("brand", site_name)
        else:
            brand_info["platform"] = site_category
        if not (brand_info.get("brand") or "").strip():
            brand_info["brand"] = site_name
        logger.info("사이트 분류: %s → %s/%s", url, site_category, site_name)
        # 매핑 있는 사이트도 site_registry에 등록 (중복이면 스킵)
        from app.core.site_classifier import register_site, _extract_domain, ingest_homepage_once

        _domain = _extract_domain(url)
        if _domain:
            await register_site(
                domain=_domain,
                category_large=site_category,
                site_name=site_name,
            )
        await ingest_homepage_once(url)
    else:
        # 미등록 사이트: LLM 분석 후 결과로 자동 등록
        from app.core.site_classifier import _extract_domain

        _domain = _extract_domain(url)
        if _domain:
            logger.info("미등록 사이트 감지: %s — LLM 분석 후 자동 등록 예정", _domain)
    ...
    if material_type == "user" and user_category_large:
        ...
        # 매핑 테이블의 분류를 LLM 결과보다 우선 (위에서 fetch 우선 반영된 site_category 사용)
        cat_large = site_category
        cat_medium = analysis.get("category_medium") or site_name
    else:
        cat_large = cat_large or "기타"
        cat_medium = cat_medium or "미분류"

    # 미등록 사이트 자동 학습: LLM 분석 결과를 site_registry에 저장
    if not site_result:
        from app.core.site_classifier import _extract_domain, register_site

        _domain = _extract_domain(url)
        if _domain:
            _learned_category = analysis.get("category_large", "종합사이트")
            _learned_name = analysis.get("category_medium", _domain)
            await register_site(
                domain=_domain,
                category_large=_learned_category,
                site_name=_learned_name,
                description=f"자동 학습: {analysis.get('title', '')[:100]}",
            )
            # 홈페이지 1회 자동 섭취
            from app.core.site_classifier import ingest_homepage_once
...
async def ingest_youtube(
    ...
    # 유튜브 채널 홈페이지 1회 자동 등록 + 섭취
    from app.core.site_classifier import register_site, ingest_homepage_once, fetch_youtube_channel_meta

    # 채널 메타에서 실제 채널 URL 사용 (한글 채널명 문제 방지)
    _channel_meta = await fetch_youtube_channel_meta(channel_name, video_url=url)
    _yt_channel_url = _channel_meta.get("channel_url") or f"https://www.youtube.com/@{channel_name}"
    _yt_domain = f"youtube.com/@{channel_name}"

    if channel_name != "미확인채널":
        await register_site(
            domain=_yt_domain,
            category_large="유튜브",
            site_name=channel_name,
            description=f"유튜브 채널: {channel_name}",
        )

        # 채널 메타 DB 업데이트
        if _channel_meta.get("follower_count") is not None:
            from app.db.database import get_db_session
            from app.db.models import SiteRegistry

            try:
                with get_db_session() as session:
                    _reg = session.query(SiteRegistry).filter_by(domain=_yt_domain).first()
                    if _reg:
                        _reg.follower_count = _channel_meta["follower_count"]
                        _reg.channel_id = _channel_meta.get("channel_id", "")
                        _reg.description = _channel_meta.get("description", "")[:500] or _reg.description
                        session.commit()
                        logger.info(
                            "채널 메타 저장: %s → 구독자 %s",
                            channel_name,
                            _channel_meta["follower_count"],
                        )
            except Exception as e:
                logger.warning("채널 메타 DB 저장 실패: %s", e)

        await ingest_homepage_once(_yt_channel_url, registry_domain=_yt_domain)
