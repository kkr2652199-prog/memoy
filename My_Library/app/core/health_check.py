    ...
    try:
        ...
        try:
            ...
            if backup_result["success"]:
                logger.info(
                    "자동 백업 완료: DB %.2fMB, Wiki %.2fMB",
                    backup_result["db_size_mb"],
                    backup_result["wiki_size_mb"],
                )
                db.add(
                    Notification(
                        type="백업",
                        message=(
                            f"자동 백업 완료 — DB: {backup_result['db_size_mb']}MB, "
                            f"Wiki: {backup_result['wiki_size_mb']}MB"
                        ),
                    )
                )
            else:
                logger.warning("자동 백업 실패: %s", backup_result["error"])
                db.add(
                    Notification(
                        type="백업실패",
                        message=f"자동 백업 실패: {backup_result['error']}",
                    )
                )
        except Exception as e:
            logger.warning("자동 백업 중 오류: %s", e)

        await auto_resolve_contradictions(db)
        db.commit()
    except Exception as e:
        db.rollback()
...
def _material_tags_add(material: Material, tag: str) -> bool:
    """tags(JSON 리스트)에 tag가 없으면 추가. 변경 시 True."""
    t = material.tags
    if t is None:
        material.tags = [tag]
        return True
    if isinstance(t, list):
        if tag in t:
            return False
        material.tags = list(t) + [tag]
        return True
    if isinstance(t, str):
        parts = [p.strip() for p in t.split(",") if p.strip()]
        if tag in parts:
            return False
        material.tags = parts + [tag] if parts else [tag]
        return True
    material.tags = [str(t), tag] if tag not in str(t) else [str(t)]
    return True


async def auto_resolve_contradictions(db: Session) -> dict:
    """unresolved 모순 중 간단한 supersession은 자동 해결하고, 나머지는 제안 생성."""
    resolved = 0
    suggested = 0
    ...
    for c in supersessions:
        new_mat = (
            db.query(Material).filter(Material.id == c.material_id_new).first()
        )
        old_mat = (
            db.query(Material)
            .filter(Material.id == c.material_id_existing)
            .first()
        )
        if not new_mat or not old_mat:
            continue
        if _material_tags_add(old_mat, "superseded"):
            db.add(old_mat)
        c.status = "auto_resolved"
        c.resolved_at = datetime.now(timezone.utc)
        c.resolution_note = (
            f"자동 해결: {new_mat.title}이(가) {old_mat.title}을(를) 대체"
        )
        db.add(c)
        resolved += 1

    # === unresolved contradiction → Judge로 자동 분류 ===
    contradictions = (
        db.query(Contradiction)
        .filter(
            Contradiction.status == "unresolved",
            Contradiction.contradiction_type == "contradiction",
        )
        .limit(20)
        .all()
    )
    from app.core.knowledge_engine import judge_contradiction

    judged = 0
    for c in contradictions:
        try:
            result = await judge_contradiction(c, db)
            ...
            logger.warning("Judge 실패 (id=%s): %s", c.id, e)
            c.status = "suggested"
            c.resolution_note = "Judge 호출 실패, 수동 확인 필요"
            db.add(c)
            suggested += 1

    # === suggested 중 미판정 건도 Judge 실행 (배치) ===
    unjudged = (
        db.query(Contradiction)
        .filter(
            Contradiction.status == "suggested",
            or_(
                Contradiction.resolution_note.is_(None),
                ~Contradiction.resolution_note.like("%Lint Judge%"),
            ),
        )
        .limit(10)
        .all()
    ...
    for c in unjudged:
        try:
            result = await judge_contradiction(c, db)
            if result["type"] not in ("genuine", "error"):
                resolved += 1
            judged += 1
        except Exception as e:
            logger.warning("Judge 실패 (id=%s): %s", c.id, e)
    ...
    logger.info(
        "모순 자동 처리: resolved=%s, suggested=%s, judged=%s",
        resolved,
        suggested,
        judged,
    )
