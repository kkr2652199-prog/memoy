...
from app.lotto3.predict_cooccur import army3_cooccur_predict
...
from app.lotto3.v12_pattern import apply_pattern_to_consensus
...
def _v12_build_sets_from_ranked(
    ...
    bypass_log_count = 0
    win_sets = get_recent_winning_sets(target_draw_no, V12_WIN_AVOID_N) if target_draw_no else []
    ...
    for cmb, t in ranked:
        ...
        if not tier1_filter(nlist):
            continue
        if not v12_pass_win_avoid(nlist, win_sets, win_st):
            continue
        chosen, t1 = cmb, t
        break
    if chosen is None and ranked:
        for cmb, t in ranked[:5]:
            if not v12_pass_win_avoid(list(cmb), win_sets, win_st):
                continue
...
def _v12_hyena_predict(
    all_predictions: list[dict],
    n_sets: int = 5,
    target_draw_no: int = 0,
    training_draws: list[dict] | None = None,
) -> list[dict]:
    """V12-A: 1군 hyena 5005 전수 파이프라인 + V12-G 패턴 합의."""
    try:
        ...
        if training_draws:
            consensus = apply_pattern_to_consensus(
                consensus, training_draws, target_draw_no=target_draw_no,
            )
        if not any(v > 0 for v in consensus.values()):
            return []
        pool_sz = 18 if target_draw_no >= 1100 else 15
        pool_nums = _v12_select_candidate_pool(consensus, pool_sz)
        if len(pool_nums) < 6:
            return []
...
def run_prediction_v12(target_draw_no: int) -> dict:
    ...
    fresh: list[dict] = []
    fresh.extend(_retag(army3_cdm_predict(training, SETS_PER_BRAIN_V12, target_draw_no), "v12_stat", "📊 베이지안CDM"))
    fresh.extend(
        _retag(
            army3_cooccur_predict(training, SETS_PER_BRAIN_V12, target_draw_no),
            "v12_run",
            "🕸️ 공동출현그래프",
        )
    )
    fresh.extend(
        _retag(
            army3_sumrange_predict(training, SETS_PER_BRAIN_V12, target_draw_no),
            "v12_offset",
            "📐 번호합범위",
        )
    )
    ...
    fresh.extend(_retag(army3_lstm_predict(training, SETS_PER_BRAIN_V12, target_draw_no), "v12_lstm", "V11LSTM두뇌"))
    fresh.extend(v12_fusion_predict(training, target_draw_no, SETS_PER_BRAIN_V12))

    hyena_sets = _v12_hyena_predict(fresh, SETS_PER_BRAIN_V12, target_draw_no, training)
...
def refresh_v12_prediction_scores_for_target_draw(target_draw_no: int) -> bool:
    """`lotto_draws`에 target 회차 당첨이 있으면 3군(v12) 예측 적중·보너스 갱신."""
    from app.lotto3.models import get_lotto3_db
