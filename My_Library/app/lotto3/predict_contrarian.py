...
_GAP10_NUMS: frozenset[int] = frozenset({32, 29, 2, 7, 16, 23, 28, 24, 44, 9})
_GAP_PAIR_BOOST: frozenset[tuple[int, int]] = frozenset({
    (1, 2),
    (1, 3),
    (1, 28),
    (2, 8),
...
D_GROUP_DRAW_NOS = frozenset({
    ...
    82, 83, 84, 85, 86, 87, 88, 89, 90, 91, 92, 93, 94, 95, 96, 97, 98, 99, 100, 102, 103, 104, 105, 106, 107, 108, 109, 110, 111, 112, 113, 114, 115, 116, 117, 118, 119, 120, 121, 122,
    ...
    163, 164, 165, 166, 167, 168, 169, 170, 171, 172, 173, 174, 175, 176, 177, 178, 179, 180, 181, 182, 184, 185, 186, 187, 188, 189, 190, 191, 192, 193, 194, 195, 196, 197, 199, 200, 201, 202, 203, 205,
    ...
    286, 287, 288, 289, 290, 291, 292, 293, 294, 295, 296, 297, 298, 299, 300, 301, 302, 303, 304, 305, 306, 307, 308, 309, 311, 312, 313, 314, 315, 316, 317, 318, 319, 320, 321, 322, 323, 324, 325, 326,
    ...
    487, 488, 489, 490, 491, 492, 493, 494, 495, 496, 497, 498, 499, 500, 501, 502, 503, 504, 505, 506, 507, 508, 509, 510, 511, 512, 513, 514, 515, 516, 517, 518, 519, 520, 521, 522, 524, 525, 526, 527,
    ...
    688, 689, 690, 691, 692, 693, 694, 695, 696, 697, 698, 699, 700, 701, 702, 703, 704, 705, 706, 707, 708, 709, 710, 711, 712, 713, 714, 715, 716, 717, 718, 719, 720, 721, 722, 723, 724, 726, 727, 728,
    ...
    769, 770, 771, 772, 773, 775, 776, 777, 778, 779, 780, 781, 782, 783, 784, 785, 786, 787, 788, 789, 790, 791, 792, 793, 794, 795, 796, 797, 798, 799, 801, 802, 803, 804, 805, 806, 807, 808, 809, 810,
    ...
    1011, 1012, 1013, 1014, 1015, 1016, 1017, 1018, 1019, 1020, 1021, 1022, 1023, 1024, 1025, 1026, 1027, 1028, 1029, 1030, 1031, 1032, 1033, 1034, 1035, 1036, 1038, 1039, 1041, 1042, 1043, 1044, 1045, 1046, 1047, 1048, 1049, 1050, 1051, 1052,
    ...
    1093, 1094, 1095, 1096, 1097, 1098, 1099, 1100, 1101, 1102, 1103, 1104, 1105, 1106, 1107, 1108, 1109, 1110, 1111, 1112, 1113, 1114, 1115, 1116, 1117, 1118, 1119, 1120, 1121, 1123, 1124, 1125, 1126, 1127, 1128, 1129, 1130, 1131, 1132, 1133,
...
def _filter_d_training(training_draws: list[dict], target_draw_no: int) -> list[dict]:
    out: list[dict] = []
    for d in training_draws:
        try:
            ...
            continue
        if dn >= target_draw_no:
            continue
        if dn in D_GROUP_DRAW_NOS:
            out.append(d)
    return out


def _calculate_gap10_weights(training_draws: list[dict], target_draw_no: int) -> dict[int, float]:
    _ = training_draws, target_draw_no
    return {n: (1.5 if n in _GAP10_NUMS else 1.0) for n in range(1, 46)}


def _calculate_d_pair_scores(training_draws: list[dict], target_draw_no: int) -> dict[tuple[int, int], float]:
    drows = _filter_d_training(training_draws, target_draw_no)
    if len(drows) < 3:
        drows = [d for d in training_draws if int(d.get("draw_no", 0) or 0) < target_draw_no]
...
def _pair_boost_score(cand: list[int], pair_scores: dict[tuple[int, int], float]) -> float:
    s = 0.0
    for a, b in combinations(sorted(cand), 2):
        s += float(pair_scores.get((a, b), 0.0))
    for a, b in combinations(sorted(cand), 2):
        if (a, b) in _GAP_PAIR_BOOST:
            s += 2.0
    return s
...
def _sum_odd_ok(nums: list[int]) -> bool:
    sm = sum(nums)
    if sm < 100 or sm > 210:
        return False
...
def _pmf_from_pairs(pair_scores: dict[tuple[int, int], float], gap_w: dict[int, float]) -> dict[int, float]:
    w = {n: 1e-6 for n in range(1, 46)}
    ...
    return {n: w[n] / tot for n in range(1, 46)}


def army3_contrarian_prob_vector(training_draws: list[dict]) -> dict[int, float]:
    """PMF for fusion compatibility."""
    if not training_draws:
        return {n: 1.0 / 45.0 for n in range(1, 46)}
    last = max(int(d["draw_no"]) for d in training_draws if d.get("draw_no") is not None)
    pair_scores = _calculate_d_pair_scores(training_draws, last + 1)
    gap_w = _calculate_gap10_weights(training_draws, last + 1)
    return _pmf_from_pairs(pair_scores, gap_w)
