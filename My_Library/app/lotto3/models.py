...
def get_lotto3_db() -> sqlite3.Connection:
...
def init_lotto3_db() -> None:
    """2군 전용 테이블 3개 신규 생성(기존 1군 테이블 수정 없음)."""
    conn = get_lotto3_db()
    try:
        conn.executescript(
            """
            CREATE TABLE IF NOT EXISTS lotto_predictions_army3 (
            ...
            CREATE INDEX IF NOT EXISTS idx_lp2_draw
                ON lotto_predictions_army3(target_draw_no);
            CREATE INDEX IF NOT EXISTS idx_lp2_brain
                ON lotto_predictions_army3(brain_tag);

            CREATE TABLE IF NOT EXISTS lotto_brain_weights_army3 (
            ...
            CREATE TABLE IF NOT EXISTS lotto_analysis_army3 (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
        ...
        for tag, weight in seeds:
            conn.execute(
                """
                INSERT OR IGNORE INTO lotto_brain_weights_army3
...
def get_miss_draws_for_army3(target_draw_no: int) -> list[dict]:
...
def get_full_draws_for_army3(target_draw_no: int) -> list[dict]:
    """V10 학습용: 1군 미당첨 필터 없이 target 미만 전체 회차.

    V9의 get_miss_draws_for_army3와 다르게, 1군 약점에 의존하지 않고
    전체 데이터를 학습 표본으로 사용.
    컷닝 방지: target_draw_no 미만만 사용.
    """
    conn = get_lotto3_db()
