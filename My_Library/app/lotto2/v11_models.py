...
- 6뇌 분리, 가중치 진화 (η=2.0)
- DB 스키마 변경 없음, 기존 테이블 + brain_tag 'v11_*' 사용
...
# V11 6뇌 (V9와 동일 구조, brain_tag만 v11_*)
V11_BRAINS = (
    "v11_stat",
    "v11_markov",
    "v11_combo",
...
def get_v11_training_draws(target_draw_no: int) -> list[dict]:
...
def get_v11_brain_weights() -> dict[str, float]:
    """V11 가중치 조회 (lotto_brain_weights_army2 테이블의 v11_* 사용).
...
def init_v11_seeds() -> None:
    """V11 6뇌 시드 가중치 INSERT (없으면)."""
    conn = _connect()
    try:
        for tag, weight in V11_SEED_WEIGHTS.items():
            conn.execute(
                "INSERT OR IGNORE INTO lotto_brain_weights_army2 (brain_tag, current_weight) VALUES (?, ?)",
                (tag, weight),
            )
        conn.commit()
    finally:
...
def update_v11_weights(
    ...
    *,
    ...
    컨닝 방지: target_draw_no 이하 채점 데이터만 사용.
    ...
    try:
        placeholders = ",".join("?" * len(V11_BRAINS))
        row = conn.execute(
            f"SELECT MAX(last_updated_draw) FROM lotto_brain_weights_army2 "
        ...
        for r in rows:
            ...
            weights_out[tag] = round(new_w, 4)
        conn.commit()
        logger.info(
            "[v11_weights] updated draw=%d weights=%s",
            target_draw_no,
            weights_out,
        )
        return {
...
def maybe_update_v11_weights_after_scoring(target_draw_no: int) -> dict:
    """당첨 확정·채점 후 2군 v11 가중치 자동 갱신 훅.

    - lotto_draws 당첨(num1) 없으면 skip
    - update_v11_weights 멱등 호출
    - lotto_predictions_army2 예측값 READ-ONLY (가중치만 UPDATE)
    """
    import logging
    ...
    result = update_v11_weights(target_draw_no, last_n=50, force=False)
    if result.get("updated"):
        logger.info(
            "[v11_weights] auto-updated draw=%d",
            target_draw_no,
        )
    else:
        logger.debug(
            "[v11_weights] skip draw=%d reason=%s",
            target_draw_no,
            result.get("reason"),
        )
    return result
