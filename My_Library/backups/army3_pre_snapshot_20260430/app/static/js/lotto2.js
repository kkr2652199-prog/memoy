...
async function loadLottoDrawList() {
  try {
    ...
    const dr = await fetch(resolveApiUrl('/api/lotto2/v11/draws?limit=2000'));
...
function renderPredictionsByBrain(drawNo, rows) {
  ...
  if (!byBrain[_currentBrainTab]) {
    ...
    _currentBrainTab = firstTag || 'v11_hyena';
  }
    ...
    try {
      ...
      if (sel) {
        ...
        if (rawPreds.length > 0) {
          ...
          // POST /predict 는 DB 저장만 하고 all_predictions 를 주지 않음(ok/cached 공통) → 목록 API로 채움
          await loadLottoDrawList();
