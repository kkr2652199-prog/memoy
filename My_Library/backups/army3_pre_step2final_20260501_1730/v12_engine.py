...
from app.lotto3.predict_markov import army3_markov_predict
...
_V11_HYENA_CONTRIBUTING: frozenset[str] = frozenset(
    {"v12_stat", "v12_run", "v12_offset", "v12_markov", "v12_combo", "v12_lstm", "v12_fusion"}
)
...
def run_prediction_v12(target_draw_no: int) -> dict:
    ...
    if existing and len(existing) >= 45:  # 기저 7×5 + hyena 5 + snake 5 = 45
        return {"status": "cached", "target_draw_no": target_draw_no, "v12_sets": len(existing)}
    ...
    fresh.extend(_retag(army3_offset_predict(training, SETS_PER_BRAIN_V12), "v12_offset", "V12오프셋두뇌"))
    ...
    return {"status": "ok", "target_draw_no": target_draw_no, "v12_sets": len(fresh)}
