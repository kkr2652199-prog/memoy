...
/** 링크 색 — 소속은 토글/기본 엣지 색상 규칙과 동일 */
function _edgeStrokeForLink(l, nodeList) {
  const rt = l.relation_type;
  if (rt === "소속") return _graphEdgeStrokeResolved("소속");
...
function _graphEdgeStrokeOtherForReveal(rt) {
  if (rt === "모순") return "#e94560";
  if (rt === "인과관계") return "#4ecdc4";
  if (rt === "관련") return "#555";
  if (rt === "언급") return "#78909C";
  if (rt === "관련주제") return "#00BCD4";
  return "#555";
}

function _graphEdgeStrokeColorForVisiblePaint(rt) {
  if (_graphEdgeToggleControlsType(rt)) {
    if (rt === "소속") return EDGE_COLORS.소속;
    if (rt === "shared_entity") return EDGE_COLORS.shared_entity;
    if (rt === "shared_concept") return EDGE_COLORS.shared_concept;
    return EDGE_COLORS.shared_topic;
  }
  return _graphEdgeStrokeOtherForReveal(rt);
}
...
function _graphEdgeStrokeResolved(rt) {
  if (rt === "소속") return EDGE_COLORS.소속;
  if (rt === "shared_entity") return EDGE_COLORS.shared_entity;
  if (rt === "shared_concept") return EDGE_COLORS.shared_concept;
  if (rt === "shared_topic") return EDGE_COLORS.shared_topic;
  return _graphEdgeStrokeOtherForReveal(rt);
}

function _graphEdgeStrokeForRelation(rt) {
  if (_graphEdgeToggleControlsType(rt)) {
    return _graphEdgeStrokeResolved(rt);
  }
  return _graphEdgeStrokeOtherForReveal(rt);
}
  ...
  _fullClusterCtx.linkG.selectAll("g.graph-edge").each(function (lnk) {
    ...
    if (sel === null) {
      ...
      if (rt === "소속") {
        ...
        line.attr("stroke", _graphEdgeStrokeColorForVisiblePaint(rt))
  ...
  if (_graphNeighborBadgeAnchorId != null && _graphNeighborBadgeByMatId && _graphNeighborBadgeByMatId.size) {
    linkWrap.each(function (lnk) {
      ...
      if (badgeNum != null) {
        ...
        if (rt === "shared_topic") {
          line.attr("stroke", "rgba(255, 215, 0, 0.35)")
            .attr("stroke-width", 1.5)
            .attr("stroke-opacity", op);
        } else if (rt === "shared_entity") {
          line.attr("stroke", "rgba(255, 182, 193, 0.5)")
            .attr("stroke-width", 1.8)
            .attr("stroke-opacity", op);
        } else if (rt === "shared_concept") {
          line.attr("stroke", "rgba(144, 238, 144, 0.5)")
            .attr("stroke-width", 1.8)
            .attr("stroke-opacity", op);
        } else if (rt === "소속") {
          ...
          line.attr("stroke", _edgeLineStroke(lnk))
            .attr("stroke-width", _edgeLineWidthAttr(lnk))
            .attr("stroke-opacity", op);
        }
        return;
      }
...
async function openGraphDetailPanelNeighborAnchor(d) {
  ...
  try {
    ...
    let neighborHtml = `<hr class="gdp-rule"><div class="gdp-subhead">이웃 자료</div>`
...
function _gdpInitGraphDetailPanelCards(body) {
  ...
  body.querySelectorAll(".gdp-neighbor-mini[data-mid]").forEach((row) => {
    const go = () => {
      const mid = Number.parseInt(row.getAttribute("data-mid"), 10);
      if (mid) openGraphDetailPanelMaterial(mid);
    };
    row.addEventListener("click", go);
    row.addEventListener("keydown", (ev) => {
...
async function _buildGraphDetailMaterialPanelHtml(id, brandLabel) {
  ...
  let neighborsHtml = "";
  if (!neighbors.length) {
    neighborsHtml = `<p class="gdp-meta gdp-meta--flush">연결된 자료가 없습니다.</p>`;
  } else {
    neighborsHtml =
  ...
  return (
    ...
    + `<button type="button" class="gdp-card-head" aria-expanded="true">`
    + `<span class="gdp-card-title">연결된 자료</span><span class="gdp-card-icon" aria-hidden="true">▼</span></button>`
  ...
  const linkWrap = linkG.selectAll("g.graph-edge-mini").data(links, (d) => d._linkKey).join("g")
    ...
    .each(function (d) {
      ...
      el.append("path")
        ...
        .attr("fill", "none")
        .attr("stroke", isBel ? "#78909C" : "#90A4AE")
        .attr("stroke-width", isBel ? 1 : 1.5)
        .attr("stroke-opacity", isBel ? 0.12 : _edgeOpacityAttr(d));
    });
...
function _hoverNode(d, nodeSel, linkWrap, labelSel, active) {
  ...
  linkWrap.each(function (lnk) {
    ...
    if (hit) {
      gg.style("display", null);
      const wBoost = 1.5;
      line.attr("stroke", _graphEdgeStrokeHoverHit(rt))
        .attr("stroke-opacity", 0.7)
        .attr("stroke-width", wBoost);
    } else {
      gg.style("display", null);
      line.attr("stroke", "rgba(255,255,255,0.08)")
        .attr("stroke-width", 0.5)
